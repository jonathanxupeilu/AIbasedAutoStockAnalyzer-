...
系统采用单阶段分析架构，具体流程如下：
graph TD
A[用户输入股票代码/名称] --> B[数据获取模块<br/>金融API接口];
B --> C[估值数据提取<br/>PE/PB/股息率];
C --> D[Prompt工程与问题框架];
D --> E[LLM逐次分析模块<br/>调用DeepSeek/GPT API];
E --> F[生成结构化分析报告];
F --> G[报告渲染与输出];

G -- Pandoc/WeasyPrint --> H[输出: 个股PDF/HTML报告];
G -- Markdown格式 --> I[输出: 个股Markdown报告];
